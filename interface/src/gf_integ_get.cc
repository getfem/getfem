...
void gf_integ_get(getfemint::mexargs_in& in,
                  getfemint::mexargs_out& out) {
  ...
  getfem::pintegration_method im = to_integ_object(in.pop());
  ...
  if (it != subc_tab.end()) {
    ...
    size_type imdim = 0;
    if (im->type() == getfem::IM_EXACT)
      imdim = im->exact_method()->dim();
    else if (im->type() == getfem::IM_APPROX)
      imdim = im->approx_method()->dim();
    subcmd->run(in, out, im, pai, imdim);
  } else
    bad_cmd(init_cmd);
