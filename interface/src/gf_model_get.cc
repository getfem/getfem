...
void build_sub_command_table(std::map<std::string, psub_command> &subc_tab) {
  ...
  sub_command
    ("rhs", 0, 0, 0, 1,
     if (md->is_complex()) out.pop().from_dcvector(md->complex_rhs());
     else                  out.pop().from_dcvector(md->real_rhs());
     );
  ...
  sub_command
    ("brick term rhs", 1, 4, 0, 1,
     ...
     if (md->is_complex())
       out.pop().from_dcvector(md->complex_brick_term_rhs(ind_brick, ind_term,
                                                          sym, ind_iter));
     else
       out.pop().from_dcvector(md->real_brick_term_rhs(ind_brick, ind_term,
                                                       sym, ind_iter));
     );
  ...
  sub_command
    ("variable", 1, 1, 0, 1,
     ...
     if (md->is_complex()) out.pop().from_dcvector(md->complex_variable(name));
     else                  out.pop().from_dcvector(md->real_variable(name));
     );
...
void gf_model_get(getfemint::mexargs_in& in,
                  getfemint::mexargs_out& out) {
