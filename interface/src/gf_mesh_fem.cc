...
build_sub_command_table(std::map<std::string, psub_command> &subc_tab) {
  ...
  sub_command
    ("clone", 1, 1, 0, 1,
     ...
     auto mfsum = dynamic_cast<getfem::mesh_fem_sum *>(mmf_in);
     auto mfprod = dynamic_cast<getfem::mesh_fem_product *>(mmf_in);
     auto mfls = dynamic_cast<getfem::mesh_fem_level_set *>(mmf_in);
     auto mfpart = dynamic_cast<getfem::partial_mesh_fem *>(mmf_in);
     auto mfglob = dynamic_cast<getfem::mesh_fem_global_function *>(mmf_in);
     if (mfsum)
       mmf = std::make_shared<getfem::mesh_fem_sum>(*mfsum);
     else if (mfprod)
       mmf = std::make_shared<getfem::mesh_fem_product>(*mfprod);
     else if (mfls) {
       auto mmfls = std::make_shared<getfem::mesh_fem_level_set>
                    (mfls->linked_mesh_level_set(), mfls->linked_mesh_fem());
       mmfls->adapt();
       mmf = mmfls;
     } else if (mfpart) {
...
void gf_mesh_fem(getfemint::mexargs_in& in,
                 getfemint::mexargs_out& out) {
  ...
  if (in.front().is_string()) {
    ...
    if (it != subc_tab.end()) {
      ...
      subcmd->run(in, out, mm, mmf, q_dim);
    } else
      bad_cmd(init_cmd);

  } else if (check_cmd("MeshFem", "MeshFem", in, out, 1, 7, 0, 1)) {
    ...
    dim_type qdim = 1;
    while (in.remaining()) {
      dim_type q = dim_type(in.pop().to_integer(1,65536));
      mi.push_back(q);
