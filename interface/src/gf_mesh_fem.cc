...
void gf_mesh_fem(getfemint::mexargs_in& in, getfemint::mexargs_out& out)
{
  ...
  if (in.front().is_string()) {
	...
	Create a mesh_fem that combines two (or more) mesh_fems

	All mesh_fem must share the same mesh (see
	FEM:INIT('interpolated_fem') to map a mesh_fem onto another).
	...
	mftab.push_back(&gfimf->mesh_fem());
      ...
      /*@INIT MESHFEM:INIT('levelset', @tmf MF, @tmls MLS)
	Create a mesh_fem that is conformal to implicit surfaces defined
	by levelsets.
	@*/
	...
	new getfem::mesh_fem_level_set(gmls->mesh_levelset(), 
      ...
      /*@INIT MESHFEM:INIT('partial', @tmf MF, @ivec DOFLST [,@ivec RCVLST])
	Build a restricted mesh_fem by keeping only a subset of the
	degrees of freedom of MF.

	If RCVLST is given, no FEM will be put on the convexes listed
	in RCVLST.
      @*/
      getfemint_mesh_fem *gmf = in.pop().to_getfemint_mesh_fem();
