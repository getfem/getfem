...
void gf_asm(getfemint::mexargs_in& m_in, getfemint::mexargs_out& m_out) {
  ...
  if (subc_tab.size() == 0) {
    ...
    /*@FUNC B = ('Nitsche contact rigid obstacle rhs', @int bnum, @tmim mim, @tmf mf_u, @dvec U, @tmf mf_obs, @dvec obs, @scalar fcoeff, @scalar r, @scalar theta, @scalar clambda, @scalar cmu)
    ...
    @*/
    sub_command
      ("Nitsche contact rigid obstacle rhs", 11, 11, 0, 1,
       int boundary_num = in.pop().to_integer();
       const getfem::mesh_im *mim = get_mim(in);
       const getfem::mesh_fem *mf_u = in.pop().to_const_mesh_fem();
       ...
       scalar_type theta = in.pop().to_scalar();
