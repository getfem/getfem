...
pde.bound{1}.R  = {sprintf('(y-%d).*(y-%d)',-R0,-R1),0};
pde.bound{2}.R  = {0,sprintf('(x-%d).*(x-%d)',R0,R1)};
pde.bound{3}.R  = {0,0};
...
Nt=8; Nr=4;
m=gf_mesh('empty',2);
dtheta=3*pi/2*1/Nt; R=R0+(R1-R0)*(0:Nr-1)/(Nr-1);
gt_order=min(Ku,3);
...
tic; [U,P,pde2]=gf_solve(pde); disp(sprintf('solve done in %.2f sec', toc));

Ul=gf_compute(pde.mf_u,U,'interpolate on',mfulag);
subplot(2,2,1); 
gf_plot(mfulag,Ul,'norm','on','deformation',Ul,'deformation_scale','5%',...
	'deformed_mesh','on');
...
hold on; gf_plot(pde.mf_p,P(:)'); hold off; 
colorbar; title('Quiver plot of U, with color plot of the pression');

subplot(2,2,4); 
gf_plot(mfulag,Ul(:)','mesh','on','quiver_density',300,'quiver_scale',0.4);%...'meshopts',{'regions',1,'curved','on'});
hold on; gf_plot(pde.mf_p,P(:)'); hold off;
axis([6 8 6 8]); title('Quiver plot zoomed');
