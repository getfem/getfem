...
m1 = gf.Mesh("cartesian", np.arange(0, 1 + 1.0 / NX, 1.0 / NX))  # Structured 1D mesh
...
m2 = gf.Mesh("triangles grid", np.arange(0, 1 + 1.0 / NX, 1.0 / NX), np.arange(0, 1 + 1.0 / NX, 1.0 / NX))  # Structured 2D mesh
...
result = gf.asm("generic", mim1, 0, "u1*Secondary_Domain(u2)", -1, md, "Secondary_domain", "secdom")
if abs(result - 0.5) > 1e-8:
    print("Bad value")
    exit(1)

result = gf.asm("generic", mim1, 0, "u1*(Secondary_Domain(Grad_u2)(1))", -1, md, "Secondary_domain", "secdom")
if abs(result - 0.5) > 1e-8:
    print("Bad value")
    exit(1)

result = gf.asm("generic", mim1, 0, "u1*(Secondary_Domain(X)(1)+Secondary_Domain(X)(2))", -1, md, "Secondary_domain", "secdom")
if abs(result - 0.5) > 1e-8:
    print("Bad value")
    exit(1)

result = gf.asm("generic", mim1, 0, "u1*sqr(Secondary_Domain(u2))", -1, md, "Secondary_domain", "secdom")
if abs(result - 7.0 / 12.0) > 1e-8:
    print("Bad value")
    exit(1)


M = gf.asm("generic", mim1, 2, "Test_u1*Secondary_Domain(Test2_u2)", -1, md, "Secondary_domain", "secdom")
