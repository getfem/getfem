...
NX = 4
m = gf.Mesh("triangles grid", np.arange(0, 1 + 1.0 / NX, 1.0 / NX), np.arange(0, 1 + 1.0 / NX, 1.0 / NX))  # Structured mesh
...
result1 = gf.asm("generic", mim, 0, "Def P(a):=a*(a'); Contract(P(Grad_v), 1, 2)", -1, md)
result2 = gf.asm("generic", mim, 0, "Def P(a):=a*(a'); Trace(P(Grad_v))", -1, md)
if abs(result1 - result2) > 1e-8:
    print("Bad value")
    exit(1)

# Constant order 3 tensor contraction test
result1 = gf.asm("generic", mim, 0, "Contract([[[1,1],[2,2]],[[1,1],[2,2]]], 1, 2)", -1, md)
...
result1 = gf.asm("generic", mim, 1, "Contract(Grad_v, 1, 2, Grad_Test_v, 1, 2)", -1, md)
...
result1 = gf.asm("generic", mim, 2, "Contract(Grad_Test2_v, 1, 2, Grad_Test_v, 1, 2)", -1, md)
result2 = gf.asm("generic", mim, 2, "Grad_Test2_v : Grad_Test_v", -1, md)
if np.linalg.norm(result1.full() - result2.full()) > 1e-8:
    print("Bad value")
    exit(1)
result1 = gf.asm("generic", mim, 2, "Contract(Grad_Test_v, 2, 1, Grad_Test2_v, 2, 1)", -1, md)
if np.linalg.norm(result1.full() - result2.full()) > 1e-8:
    print("Bad value")
    exit(1)

print('\nAssembly string "Def P(u):= Grad(u); P(Grad(u)+[1;1])" gives:')
res = gf.asm("expression analysis", "Def P(u):= Grad(u); P(Grad(u)+[1;1])", mim, 2, md)
if res != "(Hess_u)":
    print("Bad gradient")
    exit(1)
...
if res != "(Index_move_last(Contract(Hess_w, 2, Grad_w, 1), 2)+(Grad_w.Hess_w))":
    print("Bad gradient")
    exit(1)
