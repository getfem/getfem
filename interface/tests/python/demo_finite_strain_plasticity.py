...
dirichlet_str = "[{0}*{1}*x,0]".format("{0}", 10.0 / L / steps)
...
if symmetric:
    ...
    dx = L / 2
    dy = H / 2
mesh = gf.Mesh("import", "structured", 'GT="%s";ORG=[%f,%f];SIZES=[%f,%f];NSUBDIV=[%i,%i]' % (geotrans, xmin, ymin, dx, dy, N_L, N_H))
...
md.add_im_data("invCp0", mimd4)
# of the inverse right Cauchy Green tensor at the  # previous step.  #
md.set_variable("invCp0", np.tile([1, 1, 1, 0], mimd4.nbpts()))
