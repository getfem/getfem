...
NX_seed1 = np.linspace(-1.0, 0.0, int(NX / 3 + 1))
NX_seed2 = np.linspace(0.0, 1.0, int((NX - NX / 3) + 1))[1:]
NY_seed = np.linspace(-1.0, 1.0, NY + 1)
...
with open("%s/demo_phase_field_forces.dat" % resultspath, "w") as f:
    for step in range(steps):
        ...
        out = (mfu, md.variable("u"), "Displacements", mfphi, md.variable("phi"), "phi")
        for i, j in [[1, 1], [2, 2], [1, 2]]:
            sigma = gf.asm_generic(mim, 1, "{sigma}({i},{j})*Test_t".format(sigma=_sigma_, i=i, j=j), -1, md, "t", True, mfout, np.zeros(mfout.nbdof()))[md.nbdof() :]
