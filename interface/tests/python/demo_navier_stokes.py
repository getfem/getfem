...
geotrans = "GT_QK(2,2)"
m = gf.Mesh("import", "structured", "GT='%s';ORG=[%f,%f];SIZES=[%f,%f];NSUBDIV=[%i,%i]" % (geotrans, 0, H2, W1, H1, NW1, NH1))
m.merge(gf.Mesh("import", "structured", "GT='%s';ORG=[%f,%f];SIZES=[%f,%f];NSUBDIV=[%i,%i]" % (geotrans, 0, 0, W1, H2, NW1, NH2)))
m.merge(gf.Mesh("import", "structured", "GT='%s';ORG=[%f,%f];SIZES=[%f,%f];NSUBDIV=[%i,%i]" % (geotrans, W1, 0, W2, H2, NW2, NH2)))
...
kept_dofs = np.setdiff1d(np.arange(mfv_.nb_basic_dof()), mfv_.basic_dof_on_region(WALL_RG))
mfv = gf.MeshFem("partial", mfv_, kept_dofs)

mfp_ = gf.MeshFem(m, 1)
mfp_.set_classical_fem(1)
kept_dofs = np.setdiff1d(np.arange(mfp_.nb_basic_dof()), mfp_.basic_dof_on_region(OUT_RG))
...
md.add_nonlinear_term(mim, "1/dt*(v-v0).Test_v + (Grad_v0*v0).Test_v + nu*Grad_v:Grad_Test_v - f.Test_v")
...
while t < T + 1e-8:
    ...
    md.set_variable("p_in", mfp_.eval(p_in_str.format(t), globals(), locals()).flatten("F"))
    ...
    mfv.export_to_vtk("results_%i.vtk" % step, mfv, md.variable("v"), "Velocity", mfp, md.variable("p"), "Pressure")
    t += dt
    step += 1
