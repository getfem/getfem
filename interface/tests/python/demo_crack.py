...
# Mesh definition:
m = gf.Mesh("regular_simplices", -0.5 + np.arange(nx + 1) / float(nx), -0.5 + np.arange(nx + 1) / float(nx))
...
if variant == 1:  # Cutoff enrichement
    ...
    Idofs_center = np.nonzero(np.square(DOFpts[0, :]) + np.square(DOFpts[1, :]) <= 0.1 ** 2)[0]
    mf_xfem_sing = gf.MeshFem("product", mf_part_unity, mf_sing_u)
    mf_xfem_sing.set_enriched_dofs(Idofs_center)
    if variant > 2:
        Idofs_up = np.nonzero(np.square(DOFpts[0, :] + 0.125) + np.square(DOFpts[1, :] - 0.375) <= 0.1 ** 2)[0]
        Idofs_down = np.nonzero(np.square(DOFpts[0, :] + 0.125) + np.square(DOFpts[1, :] + 0.375) <= 0.1 ** 2)[0]
...
if variant == 5 or variant == 6:  # Penalisation of the jump over the first crack
    mim_bound1 = gf.MeshIm("levelset", mls, "boundary(a)", gf.Integ("IM_STRUCTURED_COMPOSITE(IM_TRIANGLE(6),3)"))
    # gf.asm_generic(mim_bound1, 0, '1', -1) # length of the crack
    md.add_linear_term(mim_bound1, "1e17*(Xfem_plus(u)-Xfem_minus(u)).(Xfem_plus(Test_u)-Xfem_minus(Test_u))")

if variant == 4 or variant == 6:  # Penalisation of the jump over the second crack
    mim_bound2 = gf.MeshIm("levelset", mls, "boundary(b)", gf.Integ("IM_STRUCTURED_COMPOSITE(IM_TRIANGLE(6),3)"))
    md.add_linear_term(mim_bound2, "1e17*(Xfem_plus(u)-Xfem_minus(u)).(Xfem_plus(Test_u)-Xfem_minus(Test_u))")
