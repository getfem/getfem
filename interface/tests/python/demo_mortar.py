...
m = gf.Mesh("cartesian", np.arange(0, 1 + 0.5 / NX, 1.0 / NX), np.arange(0, 1 + 0.5 / NX, 1.0 / NX))
...
b_out = np.array(tuple(set(tuple(r) for r in b_out.transpose()) - set(tuple(r) for r in b_border.transpose()))).transpose()
