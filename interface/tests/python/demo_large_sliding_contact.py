...
for nit in range(load_steps):
    ...
    # , "lsearch", "simplest")
    md.solve("noisy", "max_iter", max_iter, "max_res", max_res)
    ...
    mfvm1.export_to_vtk("lsc_1_%i.vtk" % nit, mfvm1, VM1, "Von Mises Stresses 1", mfu1, U1, "Displacements 1")

    lambda1 = md.variable("lambda1")
    mf_lambda1 = md.mesh_fem_of_variable("lambda1")
    sl = gf.Slice(("boundary",), mf_lambda1, CONTACT_BOUNDARY1)
    sl.export_to_vtk("lsc_1_boundary_%i.vtk" % nit, mfu1, U1, "BDisplacements 1", mf_lambda1, lambda1, "BMultiplier 1")
    ...
    if test_case not in [0, 3]:
        ...
        mfvm2.export_to_vtk("lsc_2_%i.vtk" % nit, mfvm2, VM2, "Von Mises Stresses 2", mfu2, U2, "Displacements 2")

        sl = gf.Slice(("boundary",), mfu2, CONTACT_BOUNDARY2)
        sl.export_to_vtk("lsc_2_boundary_%i.vtk" % nit, mfu2, U2, "BDisplacements 2")
