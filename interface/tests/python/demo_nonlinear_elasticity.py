...
DX = 1.0 / N1
DY = (1.0 * h) / N2
m = gf.Mesh("cartesian", np.arange(-0.5, 0.5 + DX, DX), np.arange(0.0, h + DY, DY), np.arange(-1.5, 1.5 + 3 * DX, 3 * DX))
...
if not (explicit_potential):
    ...
    _expr_1 = "{K_over_2}*sqr(log({J}))+{mu_over_2}*(Matrix_j1{be}-3)".format(K_over_2=K / 2.0, J=_J_, mu_over_2=mu / 2.0, be=_be_)

    _expr_2 = "{K_over_2}*sqr(log({J}))+{mu_over_2}*(pow(Det{be},-1./3.)*Trace{be}-3)".format(K_over_2=K / 2.0, J=_J_, mu_over_2=mu / 2.0, be=_be_)
...
def axrot_matrix(A, B, theta):
    ...
    R = np.dot(np.dot(np.dot(npla.inv(T), npla.inv(Rx)), np.dot(npla.inv(Ry), Rz)), np.dot(np.dot(Ry, Rx), T))
    return R
...
for step in range(1, nbstep + 1):
    ...
    sl.export_to_vtk("demo_nonlinear_elasticity_iter_%d.vtk" % step, "ascii", mfdu, VM, "Von Mises Stress", mfu, U, "Displacement")
