...
if d == 2:
    ...
    # corresponding constrained directions for 3d
    cunitv = [1, 0, 0, 0, 1, 0, 0, 1, 0]
...
if d == 2:
    ...
    mim_friction = gf.MeshIm(m, gf.Integ("IM_STRUCTURED_COMPOSITE(IM_TETRAHEDRON(5),4)"))
...
if version == 1 or version == 2:  # defining the matrices BN and BT by hand
    ...
    if version == 3:
        ...
        md.add_variable("lambda_t", nbc * (d - 1))
        md.add_initialized_data("friction_coeff", [friction_coeff])
        md.add_nodal_contact_with_rigid_obstacle_brick(mim, "u", "lambda_n", "lambda_t", "r", "friction_coeff", GAMMAC, obstacle, 1)
    ...
    md.add_integral_contact_with_rigid_obstacle_brick(mim_friction, "u", "lambda_n", "obstacle", "r", GAMMAC, version - 4)
    ...
    md.add_initialized_fem_data("obstacle", mfd, OBS)
    md.add_penalized_contact_with_rigid_obstacle_brick(mim_friction, "u", "obstacle", "r", GAMMAC, 2, "lambda_n")
    ...
    OBS = mfd.eval(obstacle)
    md.add_initialized_fem_data("obstacle", mfd, OBS)
    md.add_integral_contact_with_rigid_obstacle_brick(mim_friction, "u", "lambda", "obstacle", "r", "friction_coeff", GAMMAC, version - 9)
    ...
    OBS = mfd.eval(obstacle)
    md.add_initialized_fem_data("obstacle", mfd, OBS)
    md.add_penalized_contact_with_rigid_obstacle_brick(mim_friction, "u", "obstacle", "r", "friction_coeff", GAMMAC, 2, "lambda")
    ...
    for ii in range(100):
        ...
        sol = gf.linsolve_superlu(M, gf.asm_integral_contact_Uzawa_projection(GAMMAC, mim_friction, mfu, U, mflambda_partial, lambda_nt, mfd, OBS, r, friction_coeff))
