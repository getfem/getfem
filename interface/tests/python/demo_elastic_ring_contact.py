...
mesh_R = gf.Mesh("import", "structured", 'GT="%s";ORG=[-1,-1];SIZES=[2,1];NSUBDIV=[%i,%i]' % (geotrans_R, Ncirc, Nt1 + Nt2))
mesh_B = gf.Mesh("import", "structured", 'GT="%s";ORG=[%f,%f];SIZES=[%f,%f];NSUBDIV=[%i,%i]' % (geotrans_B, -lx / 2, -ly, lx, ly, Nx, Ny))
...
if not is_master_R:
    md.add_slave_contact_boundary_to_large_sliding_contact_brick(ibc, mim_R_contact, CONTACT_BOUNDARY_R, "uR", "lambda_ring", wR_str)
elif is_master_R and is_master_B:
    md.add_master_slave_contact_boundary_to_large_sliding_contact_brick(ibc, mim_R_contact, CONTACT_BOUNDARY_R, "uR", "lambda_ring", wR_str)
else:
    md.add_master_contact_boundary_to_large_sliding_contact_brick(ibc, mim_R_contact, CONTACT_BOUNDARY_R, "uR", wR_str)

if not is_master_B:
    md.add_slave_contact_boundary_to_large_sliding_contact_brick(ibc, mim_B_contact, CONTACT_BOUNDARY_B, "uB", "lambda_block", wB_str)
elif is_master_B and is_master_R:
    md.add_master_slave_contact_boundary_to_large_sliding_contact_brick(ibc, mim_B_contact, CONTACT_BOUNDARY_B, "uB", "lambda_block", wB_str)
else:
    md.add_master_contact_boundary_to_large_sliding_contact_brick(ibc, mim_B_contact, CONTACT_BOUNDARY_B, "uB", wB_str)
...
for nit in range(steps + 1):
    ...
    md.solve("noisy", "max_iter", 40, "max_res", 1e-8, "lsearch", "simplest", "alpha max ratio", 1.5, "alpha min", 0.2, "alpha mult", 0.6)[0]  # )[0]
    ...
    mfvm_R.export_to_vtk("lsc_R_%i.vtk" % nit, mfvm_R, VM_R, "Von Mises Stresses", mfu_R, U_R, "Displacements")
    ...
    sl.export_to_vtk("lsc_R_boundary_%i.vtk" % nit, mfu_R, U_R, "BDisplacements", mf_lambda_R, lambda_R, "BMultiplier")

    U_B = md.variable("uB")
    VM_B = md.compute_Von_Mises_or_Tresca("uB", lawname, "params_block", mfvm_B)
    mfvm_B.export_to_vtk("lsc_B_%i.vtk" % nit, mfvm_B, VM_B, "Von Mises Stresses", mfu_B, U_B, "Displacements")

    sl = gf.Slice(("boundary",), mfu_B, CONTACT_BOUNDARY_B)
    sl.export_to_vtk("lsc_B_boundary_%i.vtk" % nit, mfu_B, U_B, "BDisplacements")
