...
def gf_colormap(name):
    if name == "tripod":
        ...
        for i in range(1, s):
            ...
            c += [(1 - c2) * ((1 - c1) * 0.7 + c1) + c2, (1 - c2) * ((1 - c1) * 0.7) + c2 * 0.8, (1 - c2) * ((1 - c1) * 0.7) + c2 * 0.2]
    elif name == "chouette":
        c = [0.8, 1, 0.8, 0.7, 0.9, 0.4, 0.3, 0.8, 0.2, 0.1, 0.7, 0.4, 0.2, 0.7, 1.0000, 0.3, 0.3, 1.0000, 1.0, 0.8, 0.1, 1.0, 0.6, 0.1, 1.0, 0.45, 0.1, 1.0, 0.3, 0.1]
    elif name == "froid":
        c = [0.8, 1, 0.8, 0.7, 0.9, 0.4, 0.3, 0.8, 0.2, 0.1, 0.7, 0.4, 0.2, 0.7, 1.0000, 0.3, 0.3, 1.0000]
    elif name == "tank":
        c = [0, 0, 1, 0, 0.5, 1, 0, 1, 0.5, 0, 1, 0, 0.5, 1, 0, 1, 0.5, 0, 1, 0.4, 0, 1, 0, 0, 1, 0.2, 0, 1, 0.4, 0, 1, 0.6, 0, 1, 0.8, 0]
    elif name == "earth":
        c = [252, 233, 79, 247, 222, 30, 237, 212, 0, 216, 180, 0, 196, 160, 0, 138, 226, 52, 115, 210, 22, 78, 154, 6]  # Butter 1  #   Butter 2  #   Butter 3  #   Chameleon 1  #   Chameleon 2
...
class FigureItem:
    ...
    def set_scalar_data(self, data, name="scalars"):
        ...
        self.scalar_data_range = (max(m - s, self.scalar_data.min()), min(m + s, self.scalar_data.max()))
    ...
    def vtk_actors(self):
        if self.actors is None:
            ...
            if self.show_edges:
                ...
                if Pe.size:
                    E = numpy.array(numpy.concatenate((E1.transpose(), E2.transpose()), axis=0), "I")
            ...
            if self.sl.nbsplxs(1):
                ...
                tube = tvtk.TubeFilter(radius=0.4, number_of_sides=10, vary_radius="vary_radius_off", input=data)
            ...
            if self.use_scalar_bar:
                self.scalar_bar = tvtk.ScalarBarActor(title=self.scalar_data_name, orientation="horizontal", width=0.8, height=0.07)
...
class Figure:
    ...
    def show_mesh_fem(self, mf, **args):
        ...
        if "deformation" in args:
            it.deformation_from_mf(args.get("deformation_mf", mf), args["deformation"], args.get("deformation_scale", "10%"))
        if "data" in args:
