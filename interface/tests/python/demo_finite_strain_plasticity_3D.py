...
dirichlet_str = "[{0}*{1}*x,0,0]".format("{0}", 14.0 / L / steps)
...
if symmetric:
    symmetries = 2
mesh2d = gf.Mesh("import", "structured_ball", 'GT="%s";ORG=[0,0];SIZES=[%f];NSUBDIV=[%i,%i];SYMMETRIES=%i' % (geotrans2d, H / 2.0, N_R1, N_R2, symmetries))
...
if symmetric:
    ...
    kept_dofs = np.setdiff1d(np.arange(mfN.nb_basic_dof()), np.union1d(xdofsL, np.union1d(ydofsXZ, zdofsXY)))
...
# accumulated plastic strain at previous time step
md.add_im_data("gamma0", mimd1)
# Components 11, 22, 33, 12, 13, 23 of the plastic part
md.add_im_data("invCp0", mimd6)
# of the inverse right Cauchy Green tensor at the  # previous step.  #
md.set_variable("invCp0", np.tile([1, 1, 1, 0, 0, 0], mimd6.nbpts()))
...
gf.asm_define_function("hardening_law", 1, "({sigma0}+{dsigma}*(1-exp(-{delta}*t))+{H}*t)".format(sigma0=np.sqrt(2.0 / 3.0) * sigma_0, dsigma=np.sqrt(2.0 / 3.0) * (sigma_inf - sigma_0), delta=np.sqrt(2.0 / 3.0) * h1, H=2.0 / 3.0 * h2))

_K_ = "{0:.17g}".format(K)  # Bulk modulus
_mu_ = "{0:.17g}".format(mu)  # Shear modulus
ibplast = md.add_finite_strain_elastoplasticity_brick(mim, "Simo_Miehe", "displacement_and_plastic_multiplier_and_pressure", "u", "ksi", "p", "gamma0", "invCp0", _K_, _mu_, "hardening_law")
...
with open("%s/finite_strain_plasticity_forces.dat" % resultspath, "w") as f:
    for step in range(0, steps + 1):
        ...
        iters = 41
        nit = md.solve("noisy", "lsolver", "mumps", "max_iter", iters, "max_res", 1e-10, "lsearch", "simplest", "alpha max ratio", 1.5, "alpha min", 0.05, "alpha mult", 0.6)[0]  # )[0]
        if nit > 40:
            print("STEP %i HAS NOT CONVERGED, TRYING SUBSTEPPING" % (step))
            md.to_variables(backup)
            for substep in range(1, 5):
                dirichlet_data = mf_dirichlet_mult.eval(dirichlet_str.format(step - 1.0 + substep / 4.0), globals(), locals())
                md.set_variable("dirichlet_data", dirichlet_data)
                md.solve("noisy", "lsolver", "mumps", "max_iter", iters, "max_res", 1e-10, "lsearch", "simplest", "alpha max ratio", 1.5, "alpha min", 0.05, "alpha mult", 0.6)[0]  # )[0]
        print("STEP %i COMPLETED IN %f SEC" % (step, time.process_time() - starttime))

        VM = md.compute_finite_strain_elastoplasticity_Von_Mises(mim, mfout, "Simo_Miehe", "displacement_and_plastic_multiplier_and_pressure", "u", "ksi", "p", "gamma0", "invCp0", _K_, _mu_, "hardening_law")
        ...
        output = (mfu, md.variable("u"), "Displacements", mfp, md.variable("p"), "Pressure", mfksi, md.variable("ksi"), "dksi", mf_dirichlet_mult, -md.variable(dirichlet_multname), "Reaction stresses", mfout, VM, "Von Mises Stress", mfu, RHS[IU[0] : IU[0] + IU[1]], "residual u", mfp, RHS[IP[0] : IP[0] + IP[1]], "residual p", mfksi, RHS[IKSI[0] : IKSI[0] + IKSI[1]], "residual ksi")

        md.finite_strain_elastoplasticity_next_iter(mim, "Simo_Miehe", "displacement_and_plastic_multiplier_and_pressure", "u", "ksi", "p", "gamma0", "invCp0", _K_, _mu_, "hardening_law")

        GAMMA = gf.asm_generic(mim, 1, "gamma*Test_t", -1, "gamma", False, mimd1, md.variable("gamma0"), "t", True, mfout, np.zeros(mfout.nbdof()))
        ...
        if symmetric and not rigid_grip:
            ...
            f.write(("step=%i fR=(%f,%f) sR=(%f,%f)\n") % (step, dfR[0::N].sum(), dfR[1::N].sum(), dfR[0::N].sum() / H, dfR[1::N].sum() / H))
        f.flush()
