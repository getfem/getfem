...
\WEB{http://www-gmm.insa-toulouse.fr/getfem/doc}{Introduction to the
  finite element kernel and the \gf user manual}).
...
\begin{cmddescription}
  ...
  supported:
  \begin{itemize}
  \item mesh objects created with \WEB{http://www.geuz.org/gmsh}{gmsh}\index{gmsh} (GPL meshing/post processing tool): @@gf_mesh('import', 'gmsh', filename)@@. Note that gmsh meshes always use 3D points, even for planar meshes. However, you can remove the z-component of the planar mesh with @@gf_mesh_set(m, 'transform', [1 0 0; 0 1 0])@@.
...
[\tivec IDX,\tivec PID] = gf\_mesh\_get(M, 'pid from cvid'[,\tivec CVLST])
...
\begin{cmddescription}
  ...
  \sep{@@[IDX,PID]=gf\_mesh\_get(M, 'pid from cvid'[, CVLST])@@} 
can be used in order to find the points of the
  ...
  @@gf\_mesh\_fem\_get(MF, 'dof on region')@@.
...
gf\_mesh\_set(M, 'region_intersect', \tint R1, \tint R2)
gf\_mesh\_set(M, 'region_merge', \tint R1, \tint R2)
gf\_mesh\_set(M, 'region_substract', \tint R1, \tint R2)
...
\begin{cmddescription}
  ...
  \sep{@@gf\_mesh\_set(M, 'region_intersect', R1, R2)@@} : 
  replace the region number @@R1@@ with its intersection with region number @@R2@@.

  \sep{@@gf\_mesh\_set(M, 'region_merge', R1, R2)@@} : 
  merge region number @@R2@@ into region number @@R1@@.

  \sep{@@gf\_mesh\_set(M, 'region_substract', R1, R2)@@} : 
...
plot((0:n-1)/n,M);
  \end{mcode}
  \end{minipage}  \texonly{\hfill\includegraphics[width=4cm]{fempk51}}\htmlonly{\htmlimg{fempk51.png}{basis functions of the P5 fem}}
...
\tivec DOF = gf\_mesh\_fem\_get(MF, 'dof from cv', \tivec CVLST)
\tivec [DOF,CV2DOF] = gf\_mesh\_fem\_get(MF, 'dof from cv', [\tivec CVLST])
\tivec DOF = gf\_mesh\_fem\_get(MF, 'non conformal dof' [, \tivec CVLST])
...
\tmat DOF\_XY = gf\_mesh\_fem\_get(MF, 'dof nodes'[, \tivec DOFLST])
...
@@\end{synopsis}
\begin{cmddescription}
  \sep{@@gf\_mesh\_fem\_get(MF, 'nbdof')@@} : return the number of degree
  of freedom of the \tmf @@MF@@.

  \sep{@@gf\_mesh\_fem\_get(MF, 'dof from cv', CVLST)@@} :
  \index{dof}\index{degrees of freedom} return the dof IDs attached to
  the convexes listed in @@CVLST@@. WARNING: the Degree of Freedom
  might be returned in ANY order, do not use this function in your
  assembly routines. Use @@'dof from cvid'@@ instead.
  
  \sep{@@gf\_mesh\_fem\_get(MF, 'dof from cvid' [, CVLST])}@@ : return
  ...
  \sep{@@gf\_mesh\_fem\_get(MF, 'non conformal dof' [, CVLST])@@} :
  return the list of DoF which are located on the border of a convex
  ...
  common face, $a$ has a P1 FEM, and $b$ has a P2 FEM, then the dof on
  ...
  list of dof whose support is non-null on one of the regions whose
  ids are listed in @@rlist@@ (note that for boundary regions, some
  dof nodes may not lie exactly on the boundary, for example the dof
  of @@FEM_PK(n,0)@@ lies on the center of the convex, but the base
  function in not null on the convex border).

  \sep{@@gf\_mesh\_fem\_get(MF, 'dof nodes'[, DOFLST])@@} : return the
  list of interpolation points for the specified dof IDs in @@DOFLST@@
...
\end{purpose}
\begin{synopsis}
@@gf_mdstate_get('is_complex')
gf_mdstate_get('tangent_matrix')
gf_mdstate_get('constraints_matrix')
gf_mdstate_get('reduced_tangent_matrix')
gf_mdstate_get('constraints_nullspace')
gf_mdstate_get('state')
gf_mdstate_get('residual')
gf_mdstate_get('reduced_residual')
gf_mdstate_get('unreduce', \tvec U)
gf_mdstate_get('memsize')@@
\end{synopsis}
\begin{cmddescription}
\sep{@@gf_mdstate_get('is_complex')@@} :
return 0 if the model state is real, 1 if it is complex.

\sep{@@gf_mdstate_get('tangent_matrix')@@} :
return the tangent matrix stored in the model state.

\sep{@@gf_mdstate_get('constraints_matrix')@@} :
return the constraints matrix stored in the model state.

\sep{@@gf_mdstate_get('reduced_tangent_matrix')@@} : 
  return the reduced tangent matrix (i.e. the tangent matrix after
  elimination of the constraints). 

\sep{@@gf_mdstate_get('constraints_nullspace')@@} :
return the nullspace of the constraints matrix.

\sep{@@gf_mdstate_get('state')@@} :
return the vector of unknowns, which contains the solution after @@gf_mdbrick_get('solve')@@.

\sep{@@gf_mdstate_get('residual')@@} :
return the residual.

\sep{@@gf_mdstate_get('reduced_residual')@@} :
return the residual on the reduced system.
\sep{@@gf_mdstate_get('unreduce', U)@@} :
reinsert the constraint eliminated from the system.

\sep{@@gf_mdstate_get('memsize')@@} :
