...
add_initialized_fixed_size_data(const std::string &name, T e)
...
getfem::asm_stiffness_matrix_for_homogeneous_laplacian
	      (matl[0], mim, mf_u, region);
\end{cppcode}
call a standard assembly procedure for the Laplacian term defined in the file \cpp{getfem/getfem_assembling.h}. The clear method is necessary because although it is garanty that the matrices in \cpp{matl} have the good sizes they maybe not cleared before the call of \cpp{asm_real_tangent_terms}.
...
Note, that in all the cases, when a variable is add by the method \cpp{add_multiplier} of the model object, the mesh_fem will be filtered (thank to a \cpp{partial_mesh_fem_object} in order to retain only the degrees of freedom having a non vanishing contribution on the considered boundary.
...
The constraint brick allows to add an explicit constraint on a variable. Explicit means that no integration is done. if $U$ is a variable then an constraint of the type
...
indbrick = getfem::add_explicit_matrix(md, varname1, varname2, B, issymmetric = false,
	            			             iscoercive = false)
\end{cppcode}
add a brick which just add the matrix \cpp{B} to the tangent system relatively to the variables \cpp{varname1} and \cpp{varname2}. The given matrix should have has many rows as the dimension of \cpp{varname1} and as many columns as the dimension of \cpp{varname2}. If the two variables are different and if \cpp{issymmetric} is set to true then the transpose of the matrix is also added to the tangent system (default is false). set \cpp{iscoercive} to true if the term does not affect the coercivity of the tangent system (default is false). The matrix can be changed by the command
\begin{cppcode}                       
getfem::set_private_data_matrix(md, indbrick, B)
\end{cppcode}
...
\begin{cppcode}
  ind_brick = getfem::add_isotropic_linearized_elasticity_brick
              (md, mim, varname, dataname_lambda, dataname_mu,
               region = size_type(-1));
\end{cppcode}
where \cpp{dataname_lambda} and \cpp{dataname_mu} are the data of the model representing the lamé coefficients (constant or describe on a finite element method).
...
It mainly represents a mass term for transient problems but can also be used for other applications (it can be used on a boundary). Basically, this brick add a mass matrix on the tangent linear system with respect to a certain variable.
...
\end{cppcode}
where \cpp{dataname_rhos} is an optional data of the model representing the density $\rho$. If it is omitted, the density is assumed to be equal to one.
...
The principle of the $\theta$-method is to dispatch the a term $F$ into
...
\begin{cppcode}
getfem::add_basic_d_on_dt_brick(model, mim, varname, dataname_dt, dataname_rho = std::string(), region = size_type(-1));
\end{cppcode}
...
This brick represent a second order time derivative like $\rho \partial^2 u / \partial t^2$. The problem with such a term is that the $\theta$-method should be applied both on $u$ and $\partial u / \partial t$ which means that $\partial u / \partial t$ is a natural unknown of the problem. The easiest way is then to add the time derivative of the $u$ into the variables of the problem. The drawback is that one have twice as much unknown in the system. This Basic second order time derivative brick does not apply this strategy. The time derivative $\partial u / \partial t$ is considered as a data which is updated as a post-traitment (in some cases, this strategy cannot be applied is the time derivative appears to be a required unknown of the model).
...
\begin{cppcode}
  getfem::add_basic_d2_on_dt2_brick(model, mim, varname, dataname_V, dataname_dt, dataname_alpha, dataname_rho = std::string(), region = size_type(-1));
\end{cppcode}
