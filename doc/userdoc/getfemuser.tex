...
\begin{alltt}
  zcat getfem-x.xx.tar.gz | tar xvf -
  cd  getfem-x.xx
\end{alltt}
...
There is no meshing procedures in {\sc Getfem++} to mesh complex geometries. This is not the goal of this package. But you can easily load a mesh from any format (some procedures are in {\tt getfem_import.h} to load meshes from some public domain mesh generator).
...
where {\tt bgeot::simplex\_trans(N,1);} denotes the usual linear geometric transformation for simplexes of dimension N.\\[0.5cm]
...
\begin{center} \begin{tabular}{|m{0.4\linewidth}|m{0.55\linewidth}|} \hline
  ...
  {\tt mesh.structure\_of\_convex(i)\hspace{5em}->nb_faces()} & number of faces of  element of index {\tt i}. \\ \hline

  {\tt mesh.structure\_of\_convex(i)\hspace{5em}->nb\_points()} & number of vertices of  element of index {\tt i}. \\ \hline

  {\tt mesh.structure\_of\_convex(i)->dim()} & intrinsic dimension of element of index {\tt i}. \\ \hline

  {\tt mesh.structure\_of\_convex(i)\hspace{5em}->nb\_points\_of\_face(f)} & number of vertices of the face of local index {\tt f} of  element of index {\tt i}.\\ \hline
 
  {\tt mesh.structure\_of\_convex(i)\hspace{5em}->ind\_points\_of\_face(f)} & return a container with the local indexes of all vertices of the face of local index {\tt f} of  element of index {\tt i}. For instance {\tt mesh.structure\_of\_convex(i) ->ind\_points\_of\_face(f)[0]} is the local index of the first vertex. \\ \hline

  {\tt mesh.structure\_of\_convex(i)\hspace{5em}->face\_structure(f)} & gives the structure (a {\tt bgeot::pconvex\_structure}) of local index {\tt f} of  element of index {\tt i}.\\ \hline
...
{\alltt i << nn; } \\[0.5cm]
This operator takes the first index such that {\tt nn[i] == true} and remove it (it makes nn[i] = false). If {\tt nn} is empty, the returned value for {\tt i} is -1. For instance, here is the code to iterate on the points of a mesh and print it to the standard output
{\alltt
...
\begin{itemize}
\item if the fem of the $ith$ element is a real vector FEMs, {\alltt mef.get\_qdim() == mef.fem\_of\_element(i)->target\_dim()} \\ and\\ {\alltt
    mef.nb\_dof\_of\_element(i) == mef.fem\_of\_element(i).nb\_dof()}.
\item if the fem has a {\tt target\_dim} equal to $1$, {\alltt mef.nb\_dof\_of\_element(i) == mef.get\_qdim()*mef.fem\_of\_element(i).nb\_dof()}.
\end{itemize}
...
\begin{center} \begin{tabular}{|m{0.4\linewidth}|m{0.55\linewidth}|} \hline
  ...
  {\tt mef.get_qdim()} & gives the target dimension {\tt Q}.  \\ \hline
...
GMM interfaces directly {\tt std::vector<T>} so you can used it as your basic plain vector type. You can use also {\tt bgeot::vsvector<T>} form {\tt bgeot} library. If you need to interface another type of plain vector you can see in {\tt gmm_interface.h} some examples.\\
...
GMM provides two types of sparse vectors: {\tt gmm::wsvector<T>} and {\tt gmm::rsvector<T>}. {\tt gmm::wsvector<T>} is optimized for write operations and {\tt gmm::rsvector<T>} is optimized for read operations. It should be appropriate to use {\tt gmm::wsvector<T>} for assembling procedures and then to copy the vector in a {\tt gmm::rsvector<T>} for the solvers. The initialization is
\begin{alltt}
...
{\tt gmm::vect_size(V)} gives the size of a vector.

\subsubsection{matrices}
\index{GMM! gmm::row_matrix<VECT>}
\index{GMM! gmm::col_matrix<VECT>}
\index{BGEOT! bgeot::vsmatrix<T>}
GMM provides two types of matrices: {\tt gmm::row_matrix<VECT>} and {\tt gmm::col_matrix<VECT>} where {\tt VECT} should be a valid (i.e. interfaced) vector type.
Those two type of matrices store an array of {\tt VECT} so the memory is not contiguous. You can also use {\tt bgeot::vsmatrix<T>} from {\tt bgeot} library which is a plain matrix with a contiguous memory and fortran type access (column major). Initialisations are
\begin{alltt}
...
{\tt gmm::mat_nrows(M)} gives the number of rows of a matrix and {\tt gmm::mat_ncols(M)} the number of columns.
...
\end{alltt}
represents an interval whose first index is {\tt first} and length is {\tt lenght} ( for instance {\tt gmm::sub_interval(10, 3);} represents the indexes {\tt \{10, 11, 12\} }).
\index{GMM! gmm::sub_slice(first, length, step)}
\begin{alltt}
  gmm::sub_slice(first, length, step);
\end{alltt}
represents also an interval in which one index over {\tt step} is taken. ( for instance {\tt gmm::sub_slice(10, 3, 2);} represents the indexes {\tt \{10, 12, 14\} })
\begin{alltt}
  gmm::sub_index(CONT c);
\end{alltt}
represents the sub-index which is the collection of index contained in the container {\tt c}. For instance
...
Now {\tt gmm::sub_vector(V, subi)} gives a reference to a sub-vector:
\begin{alltt}
...
{\tt gmm::scale} is used to multiply a vector or a matrix with a scalar factor.
\begin{alltt}
  gmm::scale(V, 10.0);  // V * 10.0 ---> V
\end{alltt}
If one not needs to multiply the vector but wants to use the multiplied vector in an expression  {\tt gmm::scaled } gives a reference to a multiplied vector. This is only a reference, no operation is made until this reference is used somewhere. For instance
\begin{alltt}
  std::cout << gmm::scaled(V, 10.0) << std::endl;
\end{alltt}
print to the standard output the vector {\tt V} multiplied by {\tt 10.0} without changing {\tt V}.

\subsubsection{transposition}
\index{GMM! gmm::transposed}
{\tt gmm::transposed(M) } gives a reference on the transposed matrix of {\tt M}.
...
\index{GMM! gmm::vect_inf(V)}
  for vectors only, {\tt gmm::vect_norm1(V)} gives the sum of modulous of the components of {\tt V}.\\
{\tt gmm::vect_norm2(V)} is the euclidian norm of {\tt V} and {\tt gmm::vect_inf(V)} the infinity norm of {\tt V}.
...
\end{alltt}
where {\tt 2.0E-10} is the (relative) residual to be obtained to have the convergence.
Some possibilities :
\begin{alltt}
...
The solver {\tt gmm::constrained_cg(A, C, X, B, PS, PR, iter);} solve a system with linear constaints, {\tt C} is a matrix which represents the constraints.

\subsection{deeper inside GMM}

\subsubsection{iterators on vectors and matrices}
... to be done (see {\tt gmm_blas.h}).

\subsubsection{Specify and optimize a particular algorithm}
... to be done

\subsubsection{How to interface a new vector}
... to be done (see examples in {\tt gmm_interface.h}).
...
Procedures defined in the file {\tt getfem\_assembling.h} allow to assemble stiffness matrices, mass matrices and boundary conditions for a few amount of classical partial differential equation problems. All the procedures have vectors and matrices template parameters in order to be used with any matrix library.
...
is used again, but an additionnal argument has been added: {\tt nbound} is the index of the boundary in {\tt mef1} (see previous section on how to describe a boundary) where the Neumann condition is applied. Hence the assembly is done on the indicated boundary, and no more on the whole domain. {\tt mef2}  is a variable of type {\tt getfem::mesh\_fem} describing the finite element method on which $F(x)$ is defined, {\tt V} is the vector of the values of $F(x)$ on each degree of freedom of {\tt mef2}.\\[0.5cm]
...
where {\tt nbound} is the index of the boundary in {\tt mef1} where the Dirichlet condition is applied, {\tt V} is the vector of the values of $U(x)$ on each degree of freedom of {\tt mef1} and {\tt N} is the dimension for vectorial problems (should be {\tt 1} for scalar problems). This operation should be the last one because it transform the stiffness matrix {\tt RM}. It works only for Lagrange elements. At the end, one obtains the discrete system
$$ [RM] U = B, $$
...
{\tt mf\_d} are different, there is implicitly a projection (with respect to the $L^2$ norm) of the data on the finite element {\tt mf\_u}.
...
CAUTION : Linear elasticity problem is a vectorial problem, so the target dimension of {\tt mef1} (see {\tt mef.set_qdim(Q)}) should be the same as the dimension of the mesh.\\[0.5cm]
...
where {\tt M} is a matrix of any type having the right dimension (i.e. here {\tt me1.nb\_dof()}), {\tt mef} is a variable of type {\tt getfem::mesh\_fem} and should define the finite element method. {\tt mef} could represent a vectorial finite element method (i.e. {\tt mef.get_qdim } $> 1$).
It is also possible to obtain  mass matrix on a boundary with the same function:
{\alltt getfem::asm_mass\_matrix(M, mef, nbound); } \\[0.5cm]
where {\tt nbound} is the index of the boundary in {\tt mef}.
...
The input data is the vector $F$, defined on {\tt mfdata}. One wants to
...
of the first {\tt mesh_fem}, and the last one being equal to the
...
The diagonal of a tensor can be obtained with {\tt t\{:,:,3,3\}} (which is strictly equivalent to {\tt t\{1,2,3,3\}}: the colon is just here to improve the readability). This is the same operator than for permutation operations. Note that {\tt t\{:,:,1,1\}} or {\tt t\{:,:,4,4\}} are not valid operations. 
...
To be done ... please read \cite{BASCOMP} for more details and see the file {\tt getfem\_fem.C} for practical implementation.
...
{\tt getfem::asm_L2\_norm(mef, U); } \\[0.5cm]
{\tt getfem::asm_H1\_semi\_norm(mef, U); } \\[0.5cm]
{\tt getfem::asm_H1\_norm(mef, U); } \\[0.5cm]
where {\tt mef} is a variable of type {\tt getfem::mesh\_fem} and describes the finite element method on which the solution is defined, {\tt U} is the vector of values of the solution on each degree of freedom of {\tt mef}. The size of  {\tt U} should be {\tt mef.nb\_dof()}.\\[0.5cm]
...
{\tt getfem::compute\_gradient(mef1, mef2, U, V); }\\[0.5cm]
where {\tt mef1} is a variable of type {\tt getfem::mesh\_fem} and describes the finite element method on which the solution is defined, {\tt mef2} describes the finite element method to compute the gradient, {\tt U} is a vector representing the solution and should be of size {\tt mef1.nb\_dof()}, {\tt V} is the vector on which the gradient will be computed and should be of size
{\tt N * mef2.nb\_dof()} where {\tt N} is the dimension of the domain. IMPORTANT : This function only work when {\tt mef2} is a Lagrange element. This element should be, most of the time, a discontinuous lagrangian element, because for usual element (for instance {\tt getfem::PK\_discontinuous\_fem(n, k)}), the gradient is not continuous.
...
{\tt getfem::save\_solution(filename, mef, U, K); }\\[0.5cm]
where {\tt filename} is of type {\tt std::string}, {\tt mef}  is a variable of type {\tt getfem::mesh\_fem} and describes the finite element method on which the solution is defined, {\tt U} is a vector representing the solution and should be of size {\tt mef.nb\_dof()} and {\tt K} is the degree of the standard Lagrange element used to save the solution. The solution is interpolated on each element on the more classical Lagrange element of degree {\tt K} defined on this element.\\[0.5cm]
...
{\tt getfem::interpolation\_solution(mef1, mef2, U, V); }\\[0.5cm]
where {\tt mef1}  is a variable of type {\tt getfem::mesh\_fem} and describes the finite element method on which the solution is defined, {\tt mef2} is the finite element method on which the solution will be interpolate,  {\tt U} is a vector representing the solution and should be of size {\tt mef1.nb\_dof()}, {\tt V} is a vector on which the interpolation will be computed and should be of size {\tt mef2.nb\_dof()}. IMPORTANT : {\tt mef2} should be of Lagrange type for the interpolation to makes sense but the meshes linked to {\tt mef1} and {\tt mef2} may be different (and this is the interest of this function). There is no restriction for the dimension of the domain. 
