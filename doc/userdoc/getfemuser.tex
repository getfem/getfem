...
use \cpp{mf.set\_qdim(Q)} to set the target dimension (see
...
\begin{center} \texonly{\begin{tabular}{|m{0.4\linewidth}|m{0.55\linewidth}|} \hline}\htmlonly{\xmlattributes*{table}{border}\begin{tabular}{l|l}}
  ...
  \cpp{mf.fem\_of\_element(i)} & gives a descriptor on the finite element method defined on element of index \cpp{i}.  \\ \hline

  \cpp{mf.nb\_dof\_of\_element(i)} & gives the number of degrees of freedom on the element of index \cpp{i}.  \\ \hline

  \cpp{mf.ind\_dof\_of\_element(i)} & gives a container (an array) with all the global indexes of the degrees of freedom of element of index \cpp{i}.  \\ \hline

  \cpp{mf.point\_of\_dof(i, j)} & gives a \cpp{bgeot::base\_node} which represents the point associated with the dof of local index \cpp{j} on element of index \cpp{i}.  \\ \hline

  \cpp{mf.point\_of\_dof(j)} & gives a \cpp{bgeot::base\_node} which represents the point associated with the dof of global index \cpp{j}.  \\ \hline

  \cpp{mf.reference\_point\_of\_dof(i, j)} & gives a \cpp{bgeot::base\_node} which represents the point associated with the dof of local index \cpp{j} on element of index \cpp{i} in the coordinates of the reference element.  \\ \hline

  \cpp{mf.first\_convex\_of\_dof(j)} & gives the index of the first element on which the degree of freedom of global index \cpp{j} is defined.  \\ \hline
  
  \cpp{mf.nb\_dof()} & gives the total number of different degrees of freedom.  \\ \hline
  ...
  \cpp{mf.dof_on_set(i)} & Return a \cpp{dal::bit_vector} which represent the indices of dof which are in the set of convexes or the set of faces of index \cpp{i} (see the \cpp{getfem::mesh} object).  \\ \hline
...
\begin{cppcode}
  getfem::asm\_dirichlet\_constraints($\tildeH$, $\tildeR$, mim, mf\_u, mf\_mult, mf\_rh, $R$, nbound).
\end{cppcode}
...
call
\begin{cppcode}
  ncols = getfem::Dirichlet\_nullspace($\tildeH$, $\tildeN$, $\tildeR$, Ud);
\end{cppcode}
which will return a vector $U_d$ which satisfies the Dirichlet
condition, and an orthogonal basis $\tildeN$ of the kernel
of $\tildeH$. Hence, the discrete system that must be solved is
...
\begin{cppcode}
getfem::compute\_gradient(mim, mf1, mf2, U, V);
\end{cppcode}
...
\cpp{N * mf2.nb\_dof()} , with \cpp{N} the dimension of the domain. IMPORTANT : This function only works when \cpp{mf2} is a Lagrange element. This element should be, most of the time, a discontinuous Lagrangian element, because for usual element (for instance \cpp{getfem::PK\_discontinuous\_fem(n, k)}), the gradient is not continuous.
...
\modules{CAUTION: The model bricks as they are described in this section are kept for compatibility reason. They are somewhat obsolete since they are now replacecd by the model description and the modules described in the next section. }
...
The model description of \gf allows to quickly build some fem applications on complex linear or nonlinear PDE models. The principle is to propose predefined modules which can be assembled to describe a complex situation. A module can describe either an equation (Poisson equation, linear elasticity ...) or a boundary condition (Dirichlet, Neumann ...) or any relation between two variables. Once a module is written, it is possible to use it in very different situations. This allows a reusability of the produced code and the possibility of a growing library of modules. An effort as been made in order to facilitate as much as possible the definition of a new module. A module is mainly defined by its contribution in the tangent linear system to be solved.

This model description is an evolution of the model bricks of previous versions of \gf. Compared to this old system, it is more flexible, more general, allows the coupling of model in a easier way and facilitate the writing of new components.

The kernel of the model description is contained in the file \cpp{getfem_model.h}. The two main objects are \cpp{model} and \cpp{module}. Let us first describes these two objects.


\subsection{The model object}

The aim of \cpp{model} object, defined in file \cpp{getfem\_model.h}, is to globally describe a PDE model. It mainly contains three lists: a list of reference to \cpp{mesh\_fem} objects, a list of variables (related or not to the \cpp{mesh\_fem} objects) and a list of modules. The role of the \cpp{model} object is to coordinate the module and make them produce the tangent linear system (if it is needed of course, some method may only use the residu).


The \cpp{model} object is parametrized by the types of vectors and matrices used. Exemple of declaration
\begin{cppcode}
   getfem::model<sparse\_matrix\_type, writable\_sparse\_matrix\_type, vector\_type> my\_model;
\end{cppcode}
However, two predefined type can be directly used:
\begin{cppcode}
  standard\_real\_model my\_real\_model;
  standard\_complex\_model my\_complex\_model;
\end{cppcode}

The main usefull methods on a \cpp{model} object are

\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
\cpp{m.add\_fem(const mesh\_fem \&mf)} & Add a fem description to the model and return its index. \\ \hline
\cpp{m.add\_static\_variable(lenght)} & Add a variable of fixed size to the model. It size will not depend of any fem. For instance it could be some multiplier for a fixed number of constraints. \\ \hline
\cpp{m.add\_fem\_variable(fem\_num)} & Add a variable corresponding to the degrees of freedom of the fem of index \cpp{fem\_num}. \\ \hline
\cpp{m.add\_related\_variable(const getfem::model\_type::variable \&v)} & Add a variable having a special behavior. It size is computed with a given rule from the size of one or several fems. \\ \hline
\cpp{m.add\_static\_constraint(lenght)} & Add \cpp{lenght} contraints to the model. The particularity is that constraints will not be added to the tangent system bu will be integrated to a global constraint matrix. \\ \hline
\cpp{m.add\_module(module *m, i1, i2=-1, i3=-1, i4=-1)} & Add a module which links variables of indices i1, i2, i3 and i4 (the variable is ignored if it is equal to the default value -1). \\ \hline
\end{ctableau}

+ que veux dire variable ...

+ comment assembler le système ...

+ comment le faire réagir ...

\subsection{The module object}


\subsection{How to build a new module}
