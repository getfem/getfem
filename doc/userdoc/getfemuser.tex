...
The structure \cpp{getfem::getfem\_mesh } may contains also a description about a region of the mesh, such as a boundary or a set of elements. This is handled via a container of convexes and convex faces , \cpp{getfem::mesh\_region}.
...
\begin{cppcode}
  mesh.region(30).add(3);   // add convex 3 into region 30
  mesh.region(30).add(4,3); // add face 3 of convex 4 into region 30
  mesh.sup_convex(4);  // the correspounding entry will be removed from mesh.region(30)
  for (getfem::mr\_visitor i(mesh.region(30)); !i.finished(); ++i) \{
...
\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
  ...
  \cpp{mesh.optimize\_structure()} & compact the structure (renumbers points and convexes such that there is not hole in their numbering). \\ \hline
  ...
  \cpp{mesh.region(irg)} & return a \cpp{getfem::mesh\_region}. 
  The region is stored in the mesh, and can contain a set of convex
  numbers and or convex faces. \\ \hline

  \cpp{mesh.has\_region(irg)} & returns true if the region of index \cpp{irg} has been created.\\ \hline
\end{ctableau}
...
\begin{cppcode}
  getfem::mesh\_region &rg = mesh.region(2);
...
\begin{cppcode}
  for (dal::bv\_visitor i(mesh.points\_index()); !i.finished(); ++i)
...
\begin{cppcode}
  ...
  int main(int argc, char *argv[]) \{ 
    try \{ 
      ...
      mesh.read\_from\_file(std::string(argv[1])); 
...
\begin{cppcode}
  MUMPS_CFLGAS=" -I /path/to/MUMPS/include "
...
at indexes corresponding to the degrees of freedom attached to the convex of the second (\cpp{\#2}) \cpp{mesh_fem} (here, \cpp{Z = F[mfdata.ind_dof_of_element(cv)]}. The
...
To be done ... please read \cite{BASCOMP} for more details and see the file \filename{getfem\_fem.cc} for practical implementation.
...
where $u$ is a scalar field and $k$ is a positive scalar or a symmetric positive definite order two tensor field. There are two constructors for this brick. The first one is for constant scalar coefficient $k$:
\begin{alltt}
...
There are two constructors for this brick. The first one is for constant Lamé coefficients:
\begin{alltt}
...
Using that constructor, you can build the problem like with any other bricks, adding the Volumic, Neumann and Dirichlet bricks as usual (look at \cpp{elastostatic.cc} and \cpp{plasticity.cc} in tests directory for examples), and call the solver with \cpp{getfem::standard_solve(MS, final_model, iter);}.
