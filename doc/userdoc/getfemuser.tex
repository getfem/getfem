...
If you want to use a specific BLAS library, you may have to supply the necessary link flags & libs to the configure script with 
\begin{alltt}
...
This brick allows to define a Dirichlet condition on a boundary of the
domain (i.e. set the value of the unknown on this boundary $u = R$).
The Dirichlet condition is applied either adding Lagrange multipliers
or with a reduction of the linear system. This brick works for both
real or complex terms in scalar or vectorial problems. The constructor
of this brick is:

\begin{cppcode}
  getfem::mdbrick_Dirichlet<MODEL_STATE> brick(problem, bound, num_fem);
\end{cppcode}

where \cpp{problem} is the problem on which the Dirichlet condition
will be added (a generic elliptic brick for instance), \cpp{bound}
specifies on which boundary of the main mesh fem of \cpp{problem} the
Dirichlet condition is applied, \cpp{num_fem} is an optional parameter
allowing to choose a fem is the problem has several fems (0 is the
default) and \cpp{with_multipliers} is a boolean specifying if the
Dirichlet condition is applied reducing the system (the default value
of this parameter) or with Lagrange multipliers.\\
...
For vectorial elements of dimension $Q$, it is possible to describe a condition $Hu = R$ where $H$ is an $Q$x$Q$ matrix. This is done setting the optional brick parameter \cpp{H}.
...
\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
  ...
  \cpp{brick.use_multipliers(bool v)} & set how the constraints are
  taken into account. With \cpp{v = false} the linear system is
  reducted on the kernel of the constraints and with \cpp{v = true}
  some Lagrange multipliers are added to take the constraint into
  account (see section on bricks parameters for how to change the value of the parameter). \\ \hline

  \cpp{brick.H()} & give the access to the optional parameter \cpp{H}. \\ \hline
...
\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
  ...
  \cpp{brick.Q()} & gives access to the brick parameter \cpp{Q}. change the value of Q. \cpp{q} is either a real value or a vectors of type \cpp{MODEL_STATE::vector_type}. \\ \hline 
\end{ctableau}
