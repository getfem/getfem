...
\index{GMM! gmm::mat_norm2(M)}
  \cpp{gmm::vect_norm1(V)} gives the sum of modulous of the components of \cpp{V}.\\
\cpp{gmm::vect_norm2(V)} is the euclidian norm of \cpp{V} and \cpp{gmm::vect_inf(V)} the infinity norm of \cpp{V}. \cpp{gmm::mat_norm2(M)} is the euclidian norm of the matrix \cpp{M} (Fröbenius norm).

\subsubsection{trace}
\index{GMM! gmm::mat_trace(M)}
\cpp{gmm::mat_trace(M)} gives the trace of the matrix \cpp{M}.
...
\subsubsection{various preconditioners}

\index{GMM!gmm::diagonal_precond}
\index{GMM!gmm::cholesky_precond}
\index{GMM!gmm::choleskyt_precond}
\index{GMM!gmm::ilu_precond}
\index{GMM!gmm::ilut_precond}
\index{GMM!gmm::mr_approx_inverse_precond}
\begin{cppcode}
  gmm::identity_matrix P;   // No preconditioner 
  gmm::diagonal_precond<sparse_matrix_type> P(SM); // diagonal preconditioner 
  gmm::mr_approx_inverse_precond<sparse_matrix_type> P(SM, 10, 10E-17);
                                                   // preconditioner based on MR
                                                   // iterations
  gmm::cholesky_precond<sparse_matrix_type> P(SM); // incomplete (level 0) cholesky 
                                                   // preconditioner
  // incomplete cholesky with k fill-in and threshold preconditioner
  gmm::choleskyt_precond<sparse_matrix_type> P(SM, k, threshold);
  
  gmm::ilu_precond<sparse_matrix_type> P(SM); // incomplete (level 0) ilu 
                                              // preconditioner
  // incomplete LU with k fill-in and threshold preconditioner
  gmm::ilut_precond<sparse_matrix_type> P(SM, k, threshold);
\end{cppcode}
...
\index{GETFEM!Xfem}
Xfem are finite element method with a particuar enrichment with non-polynomials functions (see \cite{Xfem} for instance). The file {\tt getfem_Xfem.h} gives a support for this kind of method (version 1.5 and later). Any ($\tau$-equivalent) valid finite element method can be extended. If {\tt pf} is a valid descriptor of a finite element method, one can build a Xfem with the declaration
\begin{alltt}
  Xfem  xf(pf);
\end{alltt}
