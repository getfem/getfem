...
there is two manner to take into account the Dirichlet condition on $\Gamma_{_D}$, changing the linear system or explicitely reduce to the kernel of the Dirichlet condition. For the first manner, the following function is defined \\[0.5cm]
...
At the end, one obtain the discrete system
$$ [RM] U = B, $$
where $U$ is the discrete unknown.\\[0.5cm]
For instance, assume {\tt mesh} is a given variable of type {\tt getfem::mesh} describing a regular mesh. The following set of instruction assembles ...
