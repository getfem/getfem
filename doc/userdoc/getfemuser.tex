...
\begin{cppcode}
  const mesh_fem &getfem::classical_mesh_fem(const getfem_mesh &mesh, dim_type K);  
\end{cppcode}
to get a classical polynomial \cpp{mesh_fem} of order $K$ on the given \cpp{mesh}.
...
Note that Getfem++ includes (since release 1.7) its own version of SuperLU 3.0 ( \texttt{http://crd.lbl.gov/\verb+~+xiaoye/SuperLU/} ), hence a direct sparse solver is available out of the box.\\
...
insertion of the stiffness matrix for the linear elasticity problem,
...
its right hand side are stored in a \cpp{model\_state} structure.
...
\cpp{mesh_fem} is a vector one ($Q \ge 1$), $h(x)$ is a $Q \times Q$
...
\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
  ...
  scalar field, this is an empty vector, for a $n\times p$ matrix
...
\begin{ctableau}{|m{0.4\linewidth}|m{0.55\linewidth}|}{ll}\hline
  \cpp{brick.coeff()} & give the access to the parameter \cpp{k} (see
  apropriated section ...).  \\ \hline
