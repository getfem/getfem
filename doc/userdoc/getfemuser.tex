...
\cpp{getfem::assembling\_Dirichlet\_condition(RM, B, mef1, nbound, V, N);} \\[0.5cm]
where \cpp{nbound} is the index of the boundary in \cpp{mef1} where the Dirichlet condition is applied, \cpp{V} is the vector of the values of $U(x)$ on each degree of freedom of \cpp{mef1} and \cpp{N} is the dimension for vectorial problems (should be \cpp{1} for scalar problems). This operation should be the last one because it transform the stiffness matrix \cpp{RM}. It works only for Lagrange elements. At the end, one obtains the discrete system
\begin{equation*} [RM] U = B, \end{equation*}
...
matrix.  \index{GETFEM!getfem::Dirichlet\_null_space} Then, one should
call \\[0.5cm]\cpp{getfem::Dirichlet\_null_space($\tildeH$, N,
