...
\end{cppcode}
A non exhautive list (see \cite{GETFEMPROJECT} or \getfemfemh for exhaustive lists) of finite element methods is given by
...
\end{cppcode}
A non exhautive list (see \cite{GETFEMPROJECT} or \getfemintegrationh for exhaustive lists) of integration methods is given below.
...
solver). One has to compile \gf with the option "-D
GETFEM_PARA_LEVEL=2" to use it. With this option, each mesh used is
