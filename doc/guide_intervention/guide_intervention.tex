...
\begin{itemize}
  ...
  \item MATLABINT : (Matlab Interface) Fonctions C++ permettant la gestion de variables et d'algorithmes utilisées par les Mex files pour une utilisation sous Matlab \\[0.2cm]
\end{itemize}

\section{Organigramme simplifi\'e de l'interface MATLAB}
L'interface MATLAB repr\'esente une sur-couche de GETFEM++ compos\'ee de sources C++ faisant interface \`a des commandes MATLAB et des sources MATLAB.

\begin{center}
\includegraphics[width=17cm,angle=0]{getfemlab_orga.eps}
\end{center}

Afin de pouvoir utiliser les commandes de GETFEM sous MATLAB il faut définir les variables suivantes :
\begin{figure}
\begin{alltt}
  setenv LD\_LIBRARY\_PATH \$\{LD\_LIBRARY\_PATH\}:/usr/local/getfem/lib
  setenv MATLABPATH /usr/local/getfem/lib
\end{alltt}
\end{figure}
(Le répertoire {\tt /usr/local/getfem/lib} est donné ici comme exemple).
...
\includegraphics[width=17cm,angle=0]{files_structure2.eps}
\end{center}
...
\end{alltt}
pour bash, o\`u nom\_login est votre nom de login unix sur le site du gmm. cette commande peut \^etre ins\'er\'ee dans votre .cshrc, .bashrc ou autre. Puis vous devez créer un fichier {\tt .cvspass} dans votre home directory. Il doit contenir les deux lignes :
\begin{alltt}
  /1 :pserver:nom_login@gmmlinux2.gmm.priv:2401/var/cvs AAF2|o@
  :pserver:nom_login@gmmlinux2.gmm.priv:/var/cvs AAF2|o@
\end{alltt}
Enfin demandez l'autorisation à l'ingénieur système pour qu'il vous rajoute sur la liste des personnes ayant droit à acceder aux sources du projet.
...
\begin{alltt}
  cvs checkout getfem++/dal/
\end{alltt}
va cr\'eer dans le r\'epertoire courant la structure de r\'epertoire \begin{alltt} getfem++/dal/ \end{alltt} et y mettre les fichier correspondants, c'est \`a dire les programmes de la biblioth\`eque dal.
...
Une fois la phase de test compl\'et\'ee, quand vous \^etes s\^ur que vous n'introduisez pas de nouveaux bugs dans le projet, vous pouvez valider les changements effectués. Pour cela vous pouvez vérifier que certains modules n'ont pas été modifiés entre temps par un autre intervenant par la commande 
\begin{alltt}
  cvs diff
\end{alltt}
puis valider ces changements par la commande
\begin{alltt}
...
\begin{alltt}
  cvs checkout getfem++
  cd getfem++
  xemacs src/bgeot_poly.h (+ modifications)
  gmake
  gmake check
  cvs diff
...
\section{charte de documentation}
  ...
  Des documentation partielles existent pour les différentes parties.
  Les compléter en prenant soin de donner suffisamment d'exemples sur les
  differentes classes.
  \`a compl\'eter
