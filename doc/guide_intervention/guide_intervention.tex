...
\section{Futur organigramme simplifi\'e de GETFEMLAB}
...
Pour le premier accès effectuer alors les commandes suivantes :
\begin{alltt}
  touch ~/.cvspass
  cvs login
\end{alltt}
puis entrer son login habituel (lorsque l'on est sûr de ne plus vouloir accéder à CVS, on peut effectuer une commande ``cvs logout'').
...
\end{alltt}
ces shells chargent l'int\'egralit\'e des biblioth\`eques consid\'er\'ees et pr\'eparent un makefile pour compiler celles-ci, ainsi que les programmes de test qui lui sont li\'es. Vous pouvez encha\^\i ner plusieurs shells si vous voulez intervenir sur plusieurs biblioth\`eques dans la m\^eme session, ou si vous voulez tester les modifications apport\'ees dans des biblioth\`eques autre que celle que vous modifiez.\\[0.4cm]
...
\section{charte de programmation}
 ...
 \subsection{Sources C++}
   ...
   \begin{itemize}
     ...
     \item pour toute modification, ou nouveau module, un programme de test est souhaitable. Ce programme doit tester les fonctionnalités de la librairie et est à mettre dans le répertoire test/ de la librairie concernée. Il faut alors rédiger un shell avec extension .dotest qui sera automatiquement exécuté.
   \end{itemize}
   \`a compl\'eter
