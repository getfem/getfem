...
(:math:`U` in Fig. :ref:`fig-syslin`). Each variable will be associated to an
...
some data (in the same format than the variables). The difference between data and
variables is that a data is not an unknown of the model. The value of the data
should be provided. In some cases (nonlinear models) some variables can be
...
For instance, in the situation described in Fig. :ref:`fig-syslin`, there is four
variables in the model, namely :math:`X, Y, V` and :math:`W`. The role of the
model object will be to assemble the linear system, i.e. to fill the sub matrices
corresponding to each variable (:math:`R_{X,X}, R_{Y,Y}, R_{V,V}`, and
...
where :math:`M` is a appropriate multiplier space. The contributions to the global
linear system can be viewed in Fig. :ref:`fig-syslinDir`. The matrix :math:`B` is
the "mass matrix" between the finite element space of the variable :math:`u` and
the finite element space of the multiplier :math:`\mu`. :math:`L_{u}` is the right
end side corresponding to the data :math:`u_D`.
