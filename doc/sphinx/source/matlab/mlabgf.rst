...
* :envvar:`GEOTRANS`: geometric transformations (defines the shape/position of
  the convexes), created with ``gf_geotrans``
* :envvar:`MESH` : mesh structure (nodes, convexes, geometric transformations for
  each convex), created with ``gf_mesh``
* :envvar:`INTEG` : integration method (exact, quadrature formula...).  Although
...
* :envvar:`FEM` : the finite element method (one per convex, can be PK, QK,
  HERMITE, etc.). Created with ``gf_fem``
* :envvar:`CVSTRUCT` : stores formal information convex structures (nb. of points,
  nb. of faces which are themselves convex structures).
* :envvar:`MESHFEM` : object linked to a mesh, where each convex has been assigned
  a FEM. Created with ``gf_mesh_fem``.
* :envvar:`MESHIM` : object linked to a mesh, where each convex has been assigned
  an integration method. Created with ``gf_mesh_im``.
* :envvar:`MESHSLICE` : object linked to a mesh, very similar to a
  P1-discontinuous |mlab_mf|. Used for fast interpolation and plotting.
* :envvar:`MDBRICK` : |mlab_mbr| , an abstraction of a part of solver (for
  ...
  |mlab_mf|, |mlab_mim| etc.
* :envvar:`MDSTATE` : "model state", holds the global data for a stack of mdbricks
  (global tangent matrix, right hand side etc.).
* :envvar:`MODEL` : "model", holds the global data, variables and description of a
