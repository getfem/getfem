...
This is generally the case for Lagrange element, but not for Hermite elements (when the gradient of the functions are taken into account). When the element is not equivalent throught the geometric transformation then GETFEM++ allows to define a matrix $M$ depending on the real element (i.e. on the geometric transformation) such that base functions on the real element are defined by
...
This element is close to be \mbox{$\tau$-equivalent} but it is not. On the real element the value of the gradient on vertices will be multiplied by the gradient of the geometric transformation, This has to be corrected. The matrix $M$ is not equal to identity but is still diagonal.
