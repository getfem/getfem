...
namespace bgeot
{
  ...
  template<class T, int N> class fsmatrix : public fsvector<T, N*N>
  {
	...
	return *(begin() + c*N+l);
      ...
      fsmatrix(size_type l, size_type c) { assert((N==l) && (N==c)); }
      fsmatrix(void) {}
  };
  ...
  {
    #ifdef __GETFEM_VERIFY
    assert(v.size() == m.nrows() && v.size() == m.ncols());
    #endif
  ...
  { // mr = m1 * m2; optimisable.
    #ifdef __GETFEM_VERIFY
      assert(m1.ncols() == m2.nrows() && mr.nrows() == m1.nrows()
	                              && mr.ncols() == m2.ncols()
	                              && ((void *)(&mr) != (void *)(&m1))
	                              && ((void *)(&mr) != (void *)(&m2)));
    #endif
  ...
  { // vr = m * v; optimisable.
    #ifdef __GETFEM_VERIFY
      assert(m.ncols() == v.size() && vr.size() == m.nrows()
	                           && ((void *)(&vr) != (void *)(&v)));
    #endif
  ...
  { // mr = transp(m1) * trans(m2); optimisable.
    #ifdef __GETFEM_VERIFY
      assert(m1.nrows() == m2.ncols() && mr.nrows() == m1.ncols()
	                              && mr.ncols() == m2.nrows()
	                              && ((void *)(&mr) != (void *)(&m1))
	                              && ((void *)(&mr) != (void *)(&m2)));
    #endif
  ...
  { // vr = transp(m) * v; optimisable.
    #ifdef __GETFEM_VERIFY
      assert(m.ncols() == vr.size() && v.size() == m.nrows()
	                            && ((void *)(&vr) != (void *)(&v)));
    #endif
  ...
  { // mr = trans(m1) * m2; optimisable.
    #ifdef __GETFEM_VERIFY
      assert(m1.nrows() == m2.nrows() && mr.nrows() == m1.ncols()
	                              && mr.ncols() == m2.ncols()
	                              && ((void *)(&mr) != (void *)(&m1))
	                              && ((void *)(&mr) != (void *)(&m2)));
    #endif
			 ...
			 double EPS = 1E-12)
  {
    x = b; tmp = m;
    #ifdef __GETFEM_VERIFY
    assert((m.nrows() == m.ncols()) && (m.ncols() == b.size())
	                            && (m.ncols() == x.size()));
    #endif
  ...
  template <class MAT> void mat_decomposition_lu(const MAT &a, MAT &l, MAT &u)
  {
    ...
    #ifdef __GETFEM_VERIFY
    assert(n == a.ncols && n == l.ncols && n == u.ncols);
    #endif
    ...
    for (i = 0; i < n; i++)
    {
      ...
      assert(e != typename MAT::value_type(0));
  ...
  template <class MAT> void mat_decomposition_llt(const MAT &a, MAT &l)
  {
    ...
    for (i = 0; i < n; i++)
    {
      ...
      assert(lii != (typename MAT::value_type)(0));
    ...
    void mat_solve_llt(const MAT &l, const VECT &b, VECT &x)
  {
    ...
    #ifdef __GETFEM_VERIFY
    assert((n == l.ncols() && n == b.size() && n == x.size()));
    #endif
