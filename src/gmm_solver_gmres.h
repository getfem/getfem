...
namespace gmm {
  ...
  void gmres(const Matrix &A, Vector &x, const VectorB &b,
    ...
    typedef typename linalg_traits<Vector>::size_type size_type;
    ...
    typedef typename row_matrix<TmpVec> HMat;
    ...
    mult(M, w, r);
    Real beta = dal::abs(vect_norm2(r));
	...
	orthogonalize(KS, H[i], i);
	
	Real H_ip1_i = vect_norm2(KS[i+1]);
	H(i+1, i) = H_ip1_i;
  ...
  void gmres(const Matrix &A, Vector &x, const VectorB &b,
    ...
    modified_gram_schmidt<Vector> orth(restart, size(x));
