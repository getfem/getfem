...
namespace getfem {
  ...
  (model &md, const mesh_im &mim, const std::string &varname,
   ...
   size_type region) {
    pbrick pbr = std::make_shared<bilap_brick>();
    model::termlist tl;
  ...
  (model &md, const mesh_im &mim, const std::string &varname,
   const std::string &dataname, size_type region) {
    pbrick pbr = std::make_shared<normal_derivative_source_term_brick>();
    model::termlist tl;
  ...
  (model &md, const mesh_im &mim, const std::string &varname,
   const std::string &dataname1, const std::string &dataname2,
   size_type region) {
    pbrick pbr = std::make_shared<KL_source_term_brick>();
    model::termlist tl;
    tl.push_back(model::term_description(varname));
  ...
  (model &md, const mesh_im &mim, const std::string &varname,
   ...
   const std::string &dataname, bool R_must_be_derivated) {
    pbrick pbr = std::make_shared<normal_derivative_Dirichlet_condition_brick>(false, R_must_be_derivated);
    model::termlist tl;
  ...
  (model &md, const mesh_im &mim, const std::string &varname,
   ...
   const std::string &dataname, bool R_must_be_derivated) {
    ...
    pbrick pbr = std::make_shared<normal_derivative_Dirichlet_condition_brick>(true, R_must_be_derivated);
    model::termlist tl;
    tl.push_back(model::term_description(varname, varname, true));
    model::varnamelist vl(1, varname);
