...
if (is_automatic) 
  ...
  version = 16; % 1 : frictionless contact and the basic contact brick
              ...
              % 16 : contact with 'static' Coulomb friction and the continuous
              %     brick : Uzawa on the Lagrangian augmented by the penalization term.
              % 17 : contact with 'static' Coulomb friction and the continuous
...
elseif (version == 9) % The continuous version, Uzawa on the augmented Lagrangian
...
[nbit, converged] = gf_model_get(md, 'solve', 'max_res', residual, 'diverged_res', diverged_residual, 'max_iter', niter, 'noisy'); % , 'very noisy');
      ...
      lambda_n = (M\ gf_asm('contact Uzawa projection', GAMMAC, mim_friction, mfu, U, mflambda_partial, lambda_n, mfd, OBS, real_r))';
...
elseif (version >= 10 && version <= 15) % The continuous version with friction, Newton
...
elseif (version == 16 || version == 17) % The continuous version, Uzawa on the augmented Lagrangian with friction
  ...
  for ii=1:100
      ...
      U = gf_model_get(md, 'variable', 'u');
      lambda_old = lambda;
      lambda = (M\ gf_asm('contact with friction Uzawa projection', GAMMAC, mim_friction, mfu, U, mflambda_partial, lambda, mfd, OBS, real_r, friction_coeff, version-15))';
