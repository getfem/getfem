...
namespace getfem {
  ...
  class compare_convex_structures_ {
    const getfem_mesh& m;
  public:
    compare_convex_structures_(const getfem_mesh& m_) : m(m_) {}
  ...
  void ordered_convex_structures(const getfem_mesh& m, const dal::bit_vector& cvlist, 
    ...
    std::sort(ordered_cvlist.begin(), ordered_cvlist.end(), compare_convex_structures_(m));
  }
  ...
  class opendx_export {
    struct exported_mesh_ {
      ...
      exported_mesh_(const getfem::getfem_mesh *pm_, size_type nrefine_, bool continuous_data_) :
	nrefine(nrefine_), continuous_data(continuous_data_), pts_object_num(0), cvs_object_num(0), 
	nb_pts(0), nb_simplexes(0), sdim(0), pm(pm_)
      {}
    };
    std::vector<exported_mesh_*> exported_meshes;

    const getfem::getfem_mesh *current_mesh;
    const getfem::mesh_fem *mf_;
  ...
  private:
    ...
    exported_mesh_* export_mesh_data_no_repeat(const getfem::getfem_mesh *pm, size_type nrefine, bool continuous_data);
    exported_mesh_* export_mesh_data(const getfem::getfem_mesh *pm, size_type nrefine, bool continuous_data);
  ...
  /**
     ...
     @output the filled exported_mesh_ structure
  */
  opendx_export::exported_mesh_* opendx_export::export_mesh_data_no_repeat(const getfem::getfem_mesh *pm, 
    ...
    exported_mesh_ *em = export_mesh_data(pm,nrefine,continuous_data);
    exported_meshes.push_back(em);
    return em;
  }

  opendx_export::exported_mesh_* opendx_export::export_mesh_data(const getfem::getfem_mesh *pm, 
								 size_type nrefine, bool continuous_data) {
    typedef dal::dynamic_tree_sorted<base_node,dal::lexicographical_less<base_node,dal::approx_less<scalar_type> > > PT_TAB;
    PT_TAB pt_tab;
    exported_mesh_ *em = new exported_mesh_(pm,nrefine,continuous_data);
  ...
  opendx_export::export_solution(const getfem::mesh_fem& mf, const VECT& U, size_type nrefine, const char *field_name, bool continuous_data) {
    exported_mesh_* em = export_mesh_data_no_repeat(&mf.linked_mesh(),nrefine,continuous_data);
...
public:
  cmd_line_opts(int argc_, char *argv_[]) : carg_num(0) {
    argv.reserve(argc_);
    for (int i=1; i < argc_; ++i) argv.push_back(argv_[i]);
  }
