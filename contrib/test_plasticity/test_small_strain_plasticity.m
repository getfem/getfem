% Copyright (C) 2010-2026 Yves Renard, Farshid Dabaghi.
%
...
option = 4; % 1 : without hardening, without im_data, without plastic multiplier
            ...
            % 5 : Souza-Auricchio model with plastic multiplier (do not work)
...
do_plot = true;
use_small_strain_pl_brick = 1; % Use the (new) small strain plasticity brick
...
switch (option)
  ...
  case 3
    if (use_small_strain_pl_brick)
      ...
      if (theta == 1)
        ...
        sigma_np1 = strcat('(lambda*Trace(Sym(Grad_u)-',Epnp1, ')*Id(meshdim) + 2*mu*(Sym(Grad_u)-', Epnp1,'))');
        sigma_theta = sigma_np1;
        alpha_theta = strcat('(alphan+sqrt(2/3)*xi*(Norm(2*mu*Deviator(',Etheta,')-(2*mu+Hk)*',Eptheta,')))');
        ...
        alpha_np1 = strcat('(alphan+sqrt(2/3)*xi*(Norm(2*mu*Deviator(',Etheta,')-(2*mu+Hk)*',Eptheta,')))');
  ...
  case 4
    if (use_small_strain_pl_brick)
      ...
      alpha_theta = strcat('(max(alphan, (sqrt(3/2)*(2*mu+Hk)*alphan+Norm(',Btheta,') - sqrt(2/3)*sigma_y)/(sqrt(3/2)*(2*mu+Hk)+sqrt(2/3)*Hi)))');
      alpha_np1 = strcat('((', alpha_theta, ' - (1-theta)*alphan)/theta)');
