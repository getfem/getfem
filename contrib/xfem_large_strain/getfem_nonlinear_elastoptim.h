 ...
 template<typename VECT1, typename VECT2>
  ...
  public:
	...
	for (size_type j = 0; j < N; ++j){
      ...
      gmm::mult_add(gradU,GsEalpha,t);// on a plus besoin du contenue de GsEalpha
      
      // The term pJ[F^{-T}:d(grad U)/d(alpha)].F^{-T} - JF^{-T}.d(grad U)/d(alpha).F^{-T}
      gradU = gmm::lu_inverse(gradU);
      temp = valP[0] * J * mat_euclidean_sp(gradU_ls, gradU);
      gmm::mult(gmm::transposed(gradU), gradU_ls, GsEalpha);
      gmm::mult(GsEalpha, gmm::transposed(gradU), GsEalpha);
      gmm::add(gmm::scaled(GsEalpha,-1.0 * J * valP[0]),gmm::scaled(GsEalpha,temp),t);
 ...
 template<typename VECT1, typename VECT2>
  ...
  public:
	...
	gradU(alpha, alpha)=gradU(alpha, alpha)+ scalar_type(1);
      ...
      AHL.sigma(E, Sigma, params);
      gmm::mult_add(gradU, Sigma, t);
      // Computation of p.J.F^{-T}
      gradU =  gmm::lu_inverse(gradU);
      gmm::add(gmm::scaled(gmm::transposed(gradU), valP[0] * J),t);     
...
template<typename MAT, typename VECT1, typename VECT2> 
  ...
  (const MAT &K_, const mesh_im &mim, 
   ...
   const mesh_region &rg = mesh_region::all_convexes()) {
    ...
    if (mf_data)
      assem1.set("M(#1,#1)+=sym(comp(NonLin(#1,#2)(i,j).vGrad(#1)(:,i,j)))");
    else
      assem1.set("M(#1,#1)+=sym(comp(NonLin(#1)(i,j).vGrad(#1)(:,i,j)))");
    assem1.push_mi(mim);
    assem1.push_mf(mf_u);
    ...
    if (mf_data)
      assem2.set("M(#1,#1)+=sym(comp(NonLin(#1,#2)(i,j).vGrad(#1)(:,i,j)))");
    else
      assem2.set("M(#1,#1)+=sym(comp(NonLin(#1)(i,j).vGrad(#1)(:,i,j)))");
    assem2.push_mi(mim);
    assem21.push_mf(mf_u);
    if (mf_data) assem.push_mf(*mf_data);
